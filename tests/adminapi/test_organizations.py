...
def test_purge():
    ...
    res = n.purge(organization_id="organizationId")
    assert res is not None
    assert tr.request.method == "POST"
    assert tr.request.path == "/admin/v1/organizations/organizationId:purge"
...
@pytest.mark.asyncio
async def test_async_purge():
    tr = AsyncTestTransport()
    tr.body = """{}"""

    n = AsyncOrganizations(tr)

    res = await n.purge(organization_id="organizationId")
    assert res is not None
    assert tr.request.method == "POST"
    assert tr.request.path == "/admin/v1/organizations/organizationId:purge"
