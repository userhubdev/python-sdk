...
class Checkout:
    ...
    plans: Optional[List[CheckoutPlan]] = dataclasses.field(default_factory=list)
    ...
    def __json_encode__(self) -> Dict[str, Any]:
        ...
        if self.plans is not None:
            data["plans"] = [CheckoutPlan.__json_encode__(v) for v in self.plans]
    ...
    def __json_decode__(data: Dict[str, Any]) -> "Checkout":
        ...
        if data.get("plans") is not None:
            kwargs["plans"] = [CheckoutPlan.__json_decode__(v) for v in data["plans"]]
